 ...
 */error_reporting(6135); $Jc = !preg_match('~^(unsafe_raw)?$~', ini_get('filter.default')); if ($Jc || ini_get('filter.default_flags')) {
    foreach (['_GET', '_POST', '_COOKIE', '_SERVER']as$X) {
        $Th = filter_input_array(constant("INPUT$X"), FILTER_UNSAFE_RAW);
        if ($Th) {
            $$X = $Th;
        }
    }
...
}function idf_unescape($u)
{
    ...
    return
str_replace($Sd.$Sd, $Sd, substr($u, 1, -1));
...
{
    $_SESSION['pwds'][$ii][$N][$V] = ($_COOKIE['adminer_key'] && is_string($G) ? [encrypt_string($G, $_COOKIE['adminer_key'])] : $G);
...
}function cookie($C, $Y, $ae = 2592000)
{
    ...
    return
header("Set-Cookie: $C=".urlencode($Y).($ae ? '; expires='.gmdate('D, d M Y H:i:s', time() + $ae).' GMT' : '').'; path='.preg_replace('~\\?.*~', '', $_SERVER['REQUEST_URI']).($ba ? '; secure' : '').'; HttpOnly; SameSite=lax');
...
}function auth_url($ii, $N, $V, $m = null)
{
    ...
    return"$B[1]?".(sid() ? SID.'&' : '').($ii != 'server' || $N != '' ? urlencode($ii).'='.urlencode($N).'&' : '').'username='.urlencode($V).($m != '' ? '&db='.urlencode($m) : '').($B[2] ? "&$B[2]" : '');
}function is_ajax()
{
...
}function queries_redirect($A, $pe, $Xf)
{
    ...
    return
query_redirect($Rf, $A, $pe, $Xf, false, !$Xf, $sh);
}function format_time($Sg)
{
    return
sprintf('%.3f s', max(0, microtime(true) - $Sg));
...
{
    return
str_repeat("$yf{0,65535}", $Yd / 65535)."$yf{0,".($Yd % 65535).'}';
}function is_utf8($X)
{
...
}function input($o, $Y, $r)
{
    ...
    $Xc = (isset($_GET['select']) || $gg ? ['orig'=>'original'] : []) + $b->editFunctions($o);
    $Ia = " name='fields[$C]'";
    if ($o['type'] == 'enum') {
        echo
nbsp($Xc['']).'<td>'.$b->editInput($_GET['edit'], $o, $Ia, $Y);
    } else {
        $Kc = 0;
        foreach ($Xc
as$y=>$X) {
        ...
        if ($xd != '') {
            ...
            echo"<input type='hidden'$Ia value='0'>"."<input type='checkbox'".(in_array(strtolower($Y), ['1', 't', 'true', 'y', 'yes', 'on']) ? " checked='checked'" : '')."$Ia value='1'>";
        } elseif ($o['type'] == 'set') {
            preg_match_all("~'((?:[^']|'')*)'~", $o['length'], $he);
            foreach ($he[1]as$s=> $X) {
                $X = stripcslashes(str_replace("''", "'", $X));
            ...
            echo'<input'.((!$fd || $r === '') && preg_match('~(?<!o)int~', $o['type']) && !preg_match('~\[\]~', $o['full_type']) ? " type='number'" : '')." value='".h($Y)."'".($oe ? " data-maxlength='$oe'" : '').(preg_match('~char|binary~', $o['type']) && $oe > 20 ? " size='40'" : '')."$Ia>";
        }
    }
...
}function fields_from_edit()
{
    ...
    foreach ((array) $_POST['fields']as$y=>$X) {
        ...
        $J[$C] = ['field'=>$C, 'privileges'=>['insert'=>1, 'update'=>1], 'null'=>1, 'auto_increment'=>($y == $Vb->primary)];
    }
...
}function select_value($X, $_, $o, $rh)
{
    ...
    if (is_array($X)) {
        ...
        foreach ($X
as$Id=>$W) {
            $J .= '<tr>'.($X != array_values($X) ? '<th>'.h($Id) : '').'<td>'.select_value($W, $_, $o, $rh);
        }
...
}function is_mail($ic)
{
    ...
    return
is_string($ic) && preg_match("(^$yf(,\\s*$yf)*\$)i", $ic);
}function is_url($Q)
{
    $Ub = '[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';
...
{
    return
preg_match('~char|text|lob|geometry|point|linestring|polygon|string|bytea~', $o['type']);
...
}function edit_form($a, $p, $K, $Wh)
{
    ...
    page_header(($Wh ? 'Edit' : 'Insert'), $n, ['select'=>[$a, $dh]], $dh);
    if ($K === false) {
...
';
}global$b,$g,$Wb,$ec,$oc,$n,$Xc,$cd,$ba,$wd,$x,$ca,$Rd,$Qe,$zf,$Vg,$gd,$Ah,$Fh,$Oh,$Vh,$ia; if (!$_SERVER['REQUEST_URI']) {
    $_SERVER['REQUEST_URI'] = $_SERVER['ORIG_PATH_INFO'];
} if (!strpos($_SERVER['REQUEST_URI'], '?') && $_SERVER['QUERY_STRING'] != '') {
    $_SERVER['REQUEST_URI'] .= "?$_SERVER[QUERY_STRING]";
} $ba = $_SERVER['HTTPS'] && strcasecmp($_SERVER['HTTPS'], 'off'); @ini_set('session.use_trans_sid', false); session_cache_limiter(''); if (!defined('SID')) {
    session_name('adminer_sid');
    $F = [0, preg_replace('~\\?.*~', '', $_SERVER['REQUEST_URI']), '', $ba];
    if (version_compare(PHP_VERSION, '5.2.0') >= 0) {
        $F[] = true;
    }
    call_user_func_array('session_set_cookie_params', $F);
    session_start();
}remove_slashes([&$_GET, &$_POST, &$_COOKIE], $Jc); if (get_magic_quotes_runtime()) {
    set_magic_quotes_runtime(false);
} @set_time_limit(0); @ini_set('zend.ze1_compatibility_mode', false); @ini_set('precision', 20); function get_lang()
{
    return'en';
...
PDO
    {
        ...
        public function query($H, $Ph = false)
        {
            $I = parent::query($H);
            $this->error = '';
            if (!$I) {
...
}$Wb = []; class Min_SQL
{
    ...
    public function select($R, $M, $Z, $ad, $af = [], $z = 1, $E = 0, $Jf = false)
    {
        global$b,$x;
        $Cd = (count($ad) < count($M));
    ...
    public function delete($R, $Sf, $z = 0)
    {
        $H = 'FROM '.table($R);

        return
queries('DELETE'.($z ? limit1($H, $Sf) : " $H$Sf"));
    }

    public function update($R, $O, $Sf, $z = 0, $Ag = "\n")
    {
        $gi = [];
        foreach ($O
as$y=>$X) {
        ...
        return
queries('UPDATE'.($z ? limit1($H, $Sf) : " $H$Sf"));
    }
...
queries('INSERT INTO '.table($R).($O ? ' ('.implode(', ', array_keys($O)).")\nVALUES (".implode(', ', $O).')' : ' DEFAULT VALUES'));
    ...
    public function insertUpdate($R, $L, $Hf)
    {
        return
false;
    }
...
}$Wb['sqlite'] = 'SQLite 3'; $Wb['sqlite2'] = 'SQLite 2'; if (isset($_GET['sqlite']) || isset($_GET['sqlite2'])) {
    $Ef = [(isset($_GET['sqlite']) ? 'SQLite3' : 'SQLite'), 'PDO_SQLite'];
        ...
        if (isset($_GET['sqlite'])) {
            class Min_SQLite
            {
                ...
                public function __construct($Ic)
                {
                    $this->_link = new
SQLite3($Ic);
                    $ji = $this->_link->version();
                    $this->server_info = $ji['versionString'];
                }
...
false;
            ...
            class Min_Result
            {
                ...
                public function fetch_field()
                {
                    ...
                    return(object) ['name'=>$this->_result->columnName($e), 'type'=>$U, 'charsetnr'=>($U == SQLITE3_BLOB ? 63 : 0)];
                }
...
SQLiteDatabase($Ic);
                ...
                public function query($H, $Ph = false)
                {
                    $ve = ($Ph ? 'unbufferedQuery' : 'query');
                    $I = @$this->_link->$ve($H, SQLITE_BOTH, $n);
                    $this->error = '';
                    if (!$I) {
                        $this->error = $n;
...
Min_SQLite
        {
            ...
            public function select_db($Ic)
            {
                if (is_readable($Ic) && $this->query('ATTACH '.$this->quote(preg_match('~(^[/\\\\]|:)~', $Ic) ? $Ic : dirname($_SERVER['SCRIPT_FILENAME'])."/$Ic").' AS a')) {
...
[];
    ...
    function fields($R)
    {
        ...
        $J = [];
        $Hf = '';
        foreach (get_rows('PRAGMA table_info('.table($R).')')as$K) {
            $C = $K['name'];
            $U = strtolower($K['type']);
            $Kb = $K['dflt_value'];
            $J[$C] = ['field'=>$C, 'type'=>(preg_match('~int~i', $U) ? 'integer' : (preg_match('~char|clob|text~i', $U) ? 'text' : (preg_match('~blob~i', $U) ? 'blob' : (preg_match('~real|floa|doub~i', $U) ? 'real' : 'numeric')))), 'full_type'=>$U, 'default'=>(preg_match("~'(.*)'~", $Kb, $B) ? str_replace("''", "'", $B[1]) : ($Kb == 'NULL' ? null : $Kb)), 'null'=>!$K['notnull'], 'privileges'=>['select'=>1, 'insert'=>1, 'update'=>1], 'primary'=>$K['pk']];
            if ($K['pk']) {
                if ($Hf != '') {
                    $J[$Hf]['auto_increment'] = false;
        ...
        $Qg = $g->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));
        preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i', $Qg, $he, PREG_SET_ORDER);
        foreach ($he
as$B) {
    ...
    function indexes($R, $h = null)
    {
        ...
        $J = [];
        $Qg = $h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));
        if (preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*")++)~i', $Qg, $B)) {
            $J[''] = ['type'=>'PRIMARY', 'columns'=>[], 'lengths'=>[], 'descs'=>[]];
            preg_match_all('~((("[^"]*+")+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i', $B[1], $he, PREG_SET_ORDER);
            foreach ($he
as$B) {
        ...
        $Rg = get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($R), $h);
        foreach (get_rows('PRAGMA index_list('.table($R).')', $h)as$K) {
            $C = $K['name'];
            $v = ['type'=>($K['unique'] ? 'UNIQUE' : 'INDEX')];
            $v['lengths'] = [];
            $v['descs'] = [];
            foreach (get_rows('PRAGMA index_info('.idf_escape($C).')', $h)as$rg) {
                $v['columns'][] = $rg['name'];
                $v['descs'][] = null;
            }
            if (preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($C).' ON '.idf_escape($R), '~').' \((.*)\)$~i', $Rg[$C], $dg)) {
                preg_match_all('/("[^"]*+")+( DESC)?/', $dg[2], $he);
                foreach ($he[2]as$y=> $X) {
                    if ($X) {
                        $v['descs'][$y] = '1';
                    }
            ...
            if (!$J[''] || $v['type'] != 'UNIQUE' || $v['columns'] != $J['']['columns'] || $v['descs'] != $J['']['descs'] || !preg_match('~^sqlite_~', $C)) {
                $J[$C] = $v;
            }
        }
...
h($g->error);
    ...
    function check_sqlite_name($C)
    {
        global$g;
        $Bc = 'db|sdb|sqlite';
        if (!preg_match("~^[^\\0]*\\.($Bc)\$~", $C)) {
            $g->error = sprintf('Please use one of the extensions %s.', str_replace('|', ', ', $Bc));
...
false;
        ...
        } elseif (!recreate_table($R, $C, $c, $jf, $Pc)) {
            return
false;
        }
        if ($Ka) {
...
true;
    ...
    function recreate_table($R, $C, $p, $jf, $Pc, $w = [])
    {
        if ($R != '') {
            if (!$p) {
...
2;
                    ...
                    $f[] = $jf[$e].($v['descs'][$y] ? ' DESC' : '');
                }
                if (!$Zb[$Ld]) {
                    if ($v['type'] != 'PRIMARY' || !$If) {
                        $w[] = [$v['type'], $Ld, $f];
                    }
                }
            }
...
as$y=>$X) {
                if ($X[0] == 'PRIMARY') {
                    unset($w[$y]);
                    $Pc[] = '  PRIMARY KEY ('.implode(', ', $X[2]).')';
                }
            }
            foreach (foreign_keys($R)as$Ld=>$q) {
                foreach ($q['source']as$y=>$e) {
                    if (!$jf[$e]) {
                        continue
2;
                    }
                    $q['source'][$y] = idf_unescape($jf[$e]);
                }
                if (!isset($Pc[" $Ld"])) {
                    $Pc[] = ' '.format_foreign_key($q);
                }
            }
            queries('BEGIN');
        }
...
as$y=>$o) {
        ...
        $p = array_merge($p, array_filter($Pc));
        if (!queries('CREATE TABLE '.table($R != '' ? "adminer_$C" : $C)." (\n".implode(",\n", $p)."\n)")) {
            return
false;
        }
        if ($R != '') {
            if ($jf && !queries('INSERT INTO '.table("adminer_$C").' ('.implode(', ', $jf).') SELECT '.implode(', ', array_map('idf_escape', array_keys($jf))).' FROM '.table($R))) {
                return
false;
            }
            $Lh = [];
            foreach (triggers($R)as$Jh=>$uh) {
                $Ih = trigger($Jh);
                $Lh[] = 'CREATE TRIGGER '.idf_escape($Jh).' '.implode(' ', $uh).' ON '.table($C)."\n$Ih[Statement]";
            }
...
true;
    ...
    function index_sql($R, $U, $C, $f)
    {
        return"CREATE $U ".($U != 'INDEX' ? 'INDEX ' : '').idf_escape($C != '' ? $C : uniqid($R.'_')).' ON '.table($R)." $f";
    }
...
['Statement'=>"BEGIN\n\t;\nEND"];
        ...
        $u = '(?:[^`"\\s]+|`[^`]*`|"[^"]*")+';
        $Kh = trigger_options();
        preg_match("~^CREATE\\s+TRIGGER\\s*$u\\s*(".implode('|', $Kh['Timing']).")\\s+([a-z]+)(?:\\s+OF\\s+($u))?\\s+ON\\s*$u\\s*(?:FOR\\s+EACH\\s+ROW\\s)?(.*)~is", $g->result("SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = ".q($C)), $B);
        $Je = $B[3];

        return
['Timing'=>strtoupper($B[1]), 'Event'=>strtoupper($B[2]).($Je ? ' OF' : ''), 'Of'=>($Je[0] == '`' || $Je[0] == '"' ? idf_unescape($Je) : $Je), 'Trigger'=>$C, 'Statement'=>$B[4]];
    }
    function triggers($R)
    {
        $J = [];
        ...
        foreach (get_rows("SELECT * FROM sqlite_master WHERE type = 'trigger' AND tbl_name = ".q($R))as$K) {
            preg_match('~^CREATE\\s+TRIGGER\\s*(?:[^`"\\s]+|`[^`]*`|"[^"]*")+\\s*('.implode('|', $Kh['Timing']).')\\s*(.*)\\s+ON\\b~iU', $K['sql'], $B);
            $J[$K['name']] = [$B[1], $B[2]];
        }
...
true;
    ...
    {
        global$g;
        $J = $g->result("SELECT sql FROM sqlite_master WHERE type IN ('table', 'view') AND name = ".q($R));
...
implode(get_vals("SELECT sql || ';;\n' FROM sqlite_master WHERE type = 'trigger' AND tbl_name = ".q($R)));
    ...
    {
        $J = [];
        foreach (get_vals('PRAGMA compile_options')as$Xe) {
            list($y, $X) = explode('=', $Xe, 2);
            $J[$y] = $X;
        }
...
preg_match('~^(columns|database|drop_col|dump|indexes|move_col|sql|status|table|trigger|variables|view|view_trigger)$~', $Ec);
    ...
    $x = 'sqlite';
    $Oh = ['integer'=>0, 'real'=>0, 'numeric'=>0, 'text'=>0, 'blob'=>0];
    $Vg = array_keys($Oh);
    $Vh = [];
    $Ve = ['=', '<', '>', '<=', '>=', '!=', 'LIKE', 'LIKE %%', 'IN', 'IS NULL', 'NOT LIKE', 'NOT IN', 'IS NOT NULL', 'SQL'];
    $Xc = ['hex', 'length', 'lower', 'round', 'unixepoch', 'upper'];
    $cd = ['avg', 'count', 'count distinct', 'group_concat', 'max', 'min', 'sum'];
    $ec = [[], ['integer|real|numeric'=>'+/-', 'text'=>'||']];
}$Wb['pgsql'] = 'PostgreSQL'; if (isset($_GET['pgsql'])) {
    $Ef = ['PgSQL', 'PDO_PgSQL'];
    define('DRIVER', 'pgsql');
    if (extension_loaded('pgsql')) {
        class Min_DB
        {
            ...
            public function connect($N, $V, $G)
            {
                ...
                if ($this->_link) {
                    $ji = pg_version($this->_link);
                    $this->server_info = $ji['server'];
...
Min_PDO
        {
            ...
            public function connect($N, $V, $G)
            {
                ...
                $this->dsn("$Q dbname='".($m != '' ? addcslashes($m, "'\\") : 'postgres')."'", $V, $G);
...
as$y=>$X) {
                    $Wh[] = "$y = $X";
                    if (isset($Hf[idf_unescape($y)])) {
                        $Z[] = "$y = $X";
                    }
                }
                if (!(($Z && queries('UPDATE '.table($R).' SET '.implode(', ', $Wh).' WHERE '.implode(' AND ', $Z)) && $g->affected_rows) || queries('INSERT INTO '.table($R).' ('.implode(', ', array_keys($O)).') VALUES ('.implode(', ', $O).')'))) {
                    return
false;
                }
            }
...
true;
    ...
    {
        $J = [];
        $Ba = ['timestamp without time zone'=>'timestamp', 'timestamp with time zone'=>'timestamptz'];
...
ORDER BY a.attnum')as$K) {
            ...
            list(, $U, $Yd, $K['length'], $va, $Ea) = $B;
            $K['length'] .= $Ea;
            $bb = $U.$va;
            if (isset($Ba[$bb])) {
                $K['type'] = $Ba[$bb];
                $K['full_type'] = $K['type'].$Yd.$Ea;
            } else {
                $K['type'] = $U;
                $K['full_type'] = $K['type'].$Yd.$va.$Ea;
            }
            $K['null'] = !$K['attnotnull'];
            $K['auto_increment'] = preg_match('~^nextval\\(~i', $K['default']);
            $K['privileges'] = ['insert'=>1, 'select'=>1, 'update'=>1];
    ...
    function indexes($R, $h = null)
    {
        ...
        $J = [];
        $eh = $h->result('SELECT oid FROM pg_class WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = current_schema()) AND relname = '.q($R));
        $f = get_key_vals("SELECT attnum, attname FROM pg_attribute WHERE attrelid = $eh AND attnum > 0", $h);
        foreach (get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption , (indpred IS NOT NULL)::int as indispartial FROM pg_index i, pg_class ci WHERE i.indrelid = $eh AND ci.oid = i.indexrelid", $h)as$K) {
            $eg = $K['relname'];
            $J[$eg]['type'] = ($K['indispartial'] ? 'INDEX' : ($K['indisprimary'] ? 'PRIMARY' : ($K['indisunique'] ? 'UNIQUE' : 'INDEX')));
            $J[$eg]['columns'] = [];
            foreach (explode(' ', $K['indkey'])as$sd) {
                $J[$eg]['columns'][] = $f[$sd];
            }
            $J[$eg]['descs'] = [];
            foreach (explode(' ', $K['indoption'])as$td) {
                $J[$eg]['descs'][] = ($td & 1 ? '1' : null);
            }
            $J[$eg]['lengths'] = [];
        }
...
ORDER BY conkey, conname")as$K) {
            if (preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA', $K['definition'], $B)) {
                ...
                if (preg_match('~^(("([^"]|"")+"|[^"]+)\.)?"?("([^"]|"")+"|[^"]+)$~', $B[2], $ge)) {
                    $K['ns'] = str_replace('""', '"', preg_replace('~^"(.+)"$~', '\1', $ge[2]));
                    $K['table'] = str_replace('""', '"', preg_replace('~^"(.+)"$~', '\1', $ge[4]));
                }
                $K['target'] = array_map('trim', explode(',', $B[3]));
                $K['on_delete'] = (preg_match("~ON DELETE ($Qe)~", $B[4], $ge) ? $ge[1] : 'NO ACTION');
                $K['on_update'] = (preg_match("~ON UPDATE ($Qe)~", $B[4], $ge) ? $ge[1] : 'NO ACTION');
                $J[$K['conname']] = $K;
            }
        }
    ...
    function view($C)
    {
        ...
        return
['select'=>trim($g->result('SELECT pg_get_viewdef('.q($C).')'))];
    }
    function collations()
    {
        return
...
as$o) {
            ...
            if (!$X) {
                ...
                if ($o[0] == '') {
                    ...
                    if ($e != $X[0]) {
                        $Rf[] = 'ALTER TABLE '.table($R)." RENAME $e TO $X[0]";
                    }
                    $c[] = "ALTER $e TYPE$X[1]";
                    if (!$X[6]) {
                ...
                if ($o[0] != '' || $fi != '') {
                    $Rf[] = 'COMMENT ON COLUMN '.table($R).".$X[0] IS ".($fi != '' ? substr($fi, 9) : "''");
                }
            }
        }
        $c = array_merge($c, $Pc);
        if ($R == '') {
            array_unshift($Rf, 'CREATE TABLE '.table($C)." (\n".implode(",\n", $c)."\n)");
        } elseif ($c) {
            array_unshift($Rf, 'ALTER TABLE '.table($R)."\n".implode(",\n", $c));
        }
        if ($R != '' && $R != $C) {
            $Rf[] = 'ALTER TABLE '.table($R).' RENAME TO '.table($C);
        }
        if ($R != '' || $qb != '') {
            $Rf[] = 'COMMENT ON TABLE '.table($C).' IS '.q($qb);
        }
        if ($Ka != '') {
        }
        foreach ($Rf
...
as$X) {
            if ($X[0] != 'INDEX') {
                ...
                $Rf[] = 'CREATE INDEX '.idf_escape($X[1] != '' ? $X[1] : uniqid($R.'_')).' ON '.table($R).' ('.implode(', ', $X[2]).')';
            }
        }
        if ($i) {
            array_unshift($Rf, 'ALTER TABLE '.table($R).implode(',', $i));
        }
        if ($Xb) {
            array_unshift($Rf, 'DROP INDEX '.implode(', ', $Xb));
        }
        foreach ($Rf
as$H) {
            if (!queries($H)) {
...
true;
    ...
    function move_tables($T, $li, $lh)
    {
        foreach (array_merge($T, $li)as$R) {
            $P = table_status($R);
            if (!queries('ALTER '.strtoupper($P['Engine']).' '.table($R).' SET SCHEMA '.idf_escape($lh))) {
                return
false;
            }
        }
...
['Statement'=>'EXECUTE PROCEDURE ()'];
        ...
        $L = get_rows('SELECT t.trigger_name AS "Trigger", t.action_timing AS "Timing", (SELECT STRING_AGG(event_manipulation, \' OR \') FROM information_schema.triggers WHERE event_object_table = t.event_object_table AND trigger_name = t.trigger_name ) AS "Events", t.event_manipulation AS "Event", \'FOR EACH \' || t.action_orientation AS "Type", t.action_statement AS "Statement" FROM information_schema.triggers t WHERE t.event_object_table = '.q($R).' AND t.trigger_name = '.q($C));
...
reset($L);
    ...
    {
        $J = [];
        foreach (get_rows('SELECT * FROM information_schema.triggers WHERE event_object_table = '.q($R))as$K) {
            $J[$K['trigger_name']] = [$K['action_timing'], $K['event_manipulation']];
        }
...
0;
    ...
    {
        global$g;
        if (preg_match('~ rows=([0-9]+)~', $g->result('EXPLAIN SELECT * FROM '.idf_escape($S['Name']).($Z ? ' WHERE '.implode(' AND ', $Z) : '')), $dg)) {
            return$dg[1];
        }
...
get_vals('SELECT nspname FROM pg_namespace ORDER BY nspname');
    ...
    {
        global$g,$Oh,$Vg;
        $J = $g->query('SET search_path TO '.idf_escape($ug));
        foreach (types()as$U) {
            if (!isset($Oh[$U])) {
                $Oh[$U] = 0;
                $Vg['User types'][] = $U;
...
false;
        ...
        foreach ($p
as$Gc=>$o) {
            $qf = idf_escape($o['field']).' '.$o['full_type'].(is_null($o['default']) ? '' : " DEFAULT $o[default]").($o['attnotnull'] ? '' : ' NOT NULL');
            $kg[] = $qf;
            if (preg_match('~nextval\(\'([^\']+)\'\)~', $o['default'], $he)) {
                $Bg = $he[1];
                $Pg = reset(get_rows("SELECT * FROM $Bg"));
                $Cg[] = "CREATE SEQUENCE $Bg INCREMENT $Pg[increment_by] MINVALUE $Pg[min_value] MAXVALUE $Pg[max_value] START ".($Ka ? $Pg['last_value'] : 1)." CACHE $Pg[cache_value];";
            }
        }
        if (!empty($Cg)) {
            $J = implode("\n\n", $Cg)."\n\n$J";
        ...
        foreach ($w
as$qd=>$v) {
            switch ($v['type']) {case'UNIQUE':$kg[] = 'CONSTRAINT '.idf_escape($qd).' UNIQUE ('.implode(', ', array_map('idf_escape', $v['columns'])).')';
            break;
            case'PRIMARY':$kg[] = 'CONSTRAINT '.idf_escape($qd).' PRIMARY KEY ('.implode(', ', array_map('idf_escape', $v['columns'])).')';
            break;
        }
        }
        foreach ($Nc
as$Mc=>$Lc) {
            $kg[] = 'CONSTRAINT '.idf_escape($Mc)." $Lc[definition] ".($Lc['deferrable'] ? 'DEFERRABLE' : 'NOT DEFERRABLE');
        }
        $J .= implode(",\n    ", $kg)."\n) WITH (oids = ".($P['Oid'] ? 'true' : 'false').');';
        foreach ($w
as$qd=> $v) {
            if ($v['type'] == 'INDEX') {
                $J .= "\n\nCREATE INDEX ".idf_escape($qd).' ON '.idf_escape($P['nspname']).'.'.idf_escape($P['Name']).' USING btree ('.implode(', ', array_map('idf_escape', $v['columns'])).');';
            }
        }
        if ($P['Comment']) {
...
as$Gc=>$o) {
            if ($o['comment']) {
                $J .= "\n\nCOMMENT ON COLUMN ".idf_escape($P['nspname']).'.'.idf_escape($P['Name']).'.'.idf_escape($Gc).' IS '.q($o['comment']).';';
            }
        }
        foreach ($Lh
as$Hh=>$Gh) {
            $Ih = trigger($Hh, $P['Name']);
            $J .= "\n\nCREATE TRIGGER ".idf_escape($Ih['Trigger'])." $Ih[Timing] $Ih[Events] ON ".idf_escape($P['nspname']).'.'.idf_escape($P['Name'])." $Ih[Type] $Ih[Statement];";
        }
...
get_key_vals('SHOW ALL');
    ...
    function process_list()
    {
        ...
        return
get_rows('SELECT * FROM pg_stat_activity ORDER BY '.($g->server_info < 9.2 ? 'procpid' : 'pid'));
    }
    function show_status()
    {
    }
    ...
    function support($Ec)
    {
        ...
        return
preg_match('~^(database|table|columns|sql|indexes|comment|view|'.($g->server_info >= 9.3 ? 'materializedview|' : '').'scheme|processlist|sequence|trigger|type|variables|drop_col|kill|dump)$~', $Ec);
    }
...
queries('SELECT pg_terminate_backend('.number($X).')');
    ...
    $Oh = [];
    $Vg = [];
    foreach (['Numbers'=>['smallint'=>5, 'integer'=>10, 'bigint'=>19, 'boolean'=>1, 'numeric'=>0, 'real'=>7, 'double precision'=>16, 'money'=>20], 'Date and time'=>['date'=>13, 'time'=>17, 'timestamp'=>20, 'timestamptz'=>21, 'interval'=>0], 'Strings'=>['character'=>0, 'character varying'=>0, 'text'=>0, 'tsquery'=>0, 'tsvector'=>0, 'uuid'=>0, 'xml'=>0], 'Binary'=>['bit'=>0, 'bit varying'=>0, 'bytea'=>0], 'Network'=>['cidr'=>43, 'inet'=>43, 'macaddr'=>17, 'txid_snapshot'=>0], 'Geometry'=>['box'=>0, 'circle'=>0, 'line'=>0, 'lseg'=>0, 'path'=>0, 'point'=>0, 'polygon'=>0]]as$y=>$X) {
        $Oh += $X;
        $Vg[$y] = array_keys($X);
    }
    $Vh = [];
    $Ve = ['=', '<', '>', '<=', '>=', '!=', '~', '!~', 'LIKE', 'LIKE %%', 'ILIKE', 'ILIKE %%', 'IN', 'IS NULL', 'NOT LIKE', 'NOT IN', 'IS NOT NULL'];
    $Xc = ['char_length', 'lower', 'round', 'to_hex', 'to_timestamp', 'upper'];
    $cd = ['avg', 'count', 'count distinct', 'max', 'min', 'sum'];
    $ec = [['char'=>'md5', 'date|time'=>'now'], ['int|numeric|real|money'=>'+/-', 'date|time'=>'+ interval/- interval', 'char|text'=>'||']];
}$Wb['oracle'] = 'Oracle'; if (isset($_GET['oracle'])) {
    $Ef = ['OCI8', 'PDO_OCI'];
...
[];
    ...
    function table_status($C = '')
    {
        $J = [];
        $wg = q($C);
        foreach (get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = '.q(DB).($C != '' ? " AND table_name = $wg" : '')."
...
true;
    ...
    {
        $J = [];
        foreach (get_rows('SELECT * FROM all_tab_columns WHERE table_name = '.q($R).' ORDER BY column_id')as$K) {
            $U = $K['DATA_TYPE'];
            $Yd = "$K[DATA_PRECISION],$K[DATA_SCALE]";
            if ($Yd == ',') {
                $Yd = $K['DATA_LENGTH'];
            }
            $J[$K['COLUMN_NAME']] = ['field'=>$K['COLUMN_NAME'], 'full_type'=>$U.($Yd ? "($Yd)" : ''), 'type'=>strtolower($U), 'length'=>$Yd, 'default'=>$K['DATA_DEFAULT'], 'null'=>($K['NULLABLE'] == 'Y'), 'privileges'=>['insert'=>1, 'select'=>1, 'update'=>1]];
        }
...
ORDER BY uc.constraint_type, uic.column_position', $h)as$K) {
            $qd = $K['INDEX_NAME'];
            $J[$qd]['type'] = ($K['CONSTRAINT_TYPE'] == 'P' ? 'PRIMARY' : ($K['CONSTRAINT_TYPE'] == 'U' ? 'UNIQUE' : 'INDEX'));
            $J[$qd]['columns'][] = $K['COLUMN_NAME'];
            $J[$qd]['lengths'][] = ($K['CHAR_LENGTH'] && $K['CHAR_LENGTH'] != $K['COLUMN_LENGTH'] ? $K['CHAR_LENGTH'] : null);
            $J[$qd]['descs'][] = ($K['DESCEND'] ? '1' : null);
        }
...
queries('CREATE TABLE '.table($C)." (\n".implode(",\n", $c)."\n)");
        ...
        return(!$c || queries('ALTER TABLE '.table($R)."\n".implode("\n", $c))) && (!$Xb || queries('ALTER TABLE '.table($R).' DROP ('.implode(', ', $Xb).')')) && ($R == $C || queries('ALTER TABLE '.table($R).' RENAME TO '.table($C)));
    }
    function foreign_keys($R)
    {
        $J = [];
...
get_vals("SELECT DISTINCT owner FROM dba_segments WHERE owner IN (SELECT username FROM dba_users WHERE default_tablespace NOT IN ('SYSTEM','SYSAUX'))");
    ...
    function set_schema($vg)
    {
        ...
        return$g->query('ALTER SESSION SET CURRENT_SCHEMA = '.idf_escape($vg));
    }
    function show_variables()
    {
        return
...
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~', $Ec);
    ...
    $Oh = [];
    $Vg = [];
    foreach (['Numbers'=>['number'=>38, 'binary_float'=>12, 'binary_double'=>21], 'Date and time'=>['date'=>10, 'timestamp'=>29, 'interval year'=>12, 'interval day'=>28], 'Strings'=>['char'=>2000, 'varchar2'=>4000, 'nchar'=>2000, 'nvarchar2'=>4000, 'clob'=>4294967295, 'nclob'=>4294967295], 'Binary'=>['raw'=>2000, 'long raw'=>2147483648, 'blob'=>4294967295, 'bfile'=>4294967296]]as$y=>$X) {
        $Oh += $X;
        $Vg[$y] = array_keys($X);
    }
    $Vh = [];
    $Ve = ['=', '<', '>', '<=', '>=', '!=', 'LIKE', 'LIKE %%', 'IN', 'IS NULL', 'NOT LIKE', 'NOT REGEXP', 'NOT IN', 'IS NOT NULL', 'SQL'];
    $Xc = ['length', 'lower', 'round', 'upper'];
    $cd = ['avg', 'count', 'count distinct', 'max', 'min', 'sum'];
    $ec = [['date'=>'current_date', 'timestamp'=>'current_timestamp'], ['number|float|double'=>'+/-', 'date|timestamp'=>'+ interval/- interval', 'char|clob'=>'||']];
}$Wb['mssql'] = 'MS SQL'; if (isset($_GET['mssql'])) {
    $Ef = ['SQLSRV', 'MSSQL', 'PDO_DBLIB'];
    define('DRIVER', 'mssql');
    if (extension_loaded('sqlsrv')) {
        class Min_DB
        {
            ...
            public function connect($N, $V, $G)
            {
                $this->_link = @sqlsrv_connect($N, ['UID'=>$V, 'PWD'=>$G, 'CharacterSet'=>'UTF-8']);
                if ($this->_link) {
                    $ud = sqlsrv_server_info($this->_link);
                    $this->server_info = $ud['SQLServerVersion'];
                } else {
                    $this->_get_error();
                }
...
Min_PDO
        {
            ...
            public function connect($N, $V, $G)
            {
                $this->dsn('dblib:charset=utf8;host='.str_replace(':', ';unix_socket=', preg_replace('~:(\\d)~', ';port=\\1', $N)), $V, $G);
...
as$y=>$X) {
                ...
                if (!queries('MERGE '.table($R).' USING (VALUES('.implode(', ', $O).')) AS source (c'.implode(', c', range(1, count($O))).') ON '.implode(' AND ', $Z).' WHEN MATCHED THEN UPDATE SET '.implode(', ', $Wh).' WHEN NOT MATCHED THEN INSERT ('.implode(', ', array_keys($O)).') VALUES ('.implode(', ', $O).');')) {
                    return
false;
                }
            }
...
WHERE o.schema_id = SCHEMA_ID('.q(get_schema()).") AND o.type IN ('S', 'U', 'V') AND o.name = ".q($R))as$K) {
            $U = $K['type'];
            $Yd = (preg_match('~char|binary~', $U) ? $K['max_length'] : ($U == 'decimal' ? "$K[precision],$K[scale]" : ''));
            $J[$K['name']] = ['field'=>$K['name'], 'full_type'=>$U.($Yd ? "($Yd)" : ''), 'type'=>$U, 'length'=>$Yd, 'default'=>$K['default'], 'null'=>$K['is_nullable'], 'auto_increment'=>$K['is_identity'], 'collation'=>$K['collation_name'], 'privileges'=>['insert'=>1, 'select'=>1, 'update'=>1], 'primary'=>$K['is_identity']];
        }
...
as$o) {
            ...
            if (!$X) {
                ...
                if ($o[0] == '') {
                    $c['ADD'][] = "\n  ".implode('', $X).($R == '' ? substr($Pc[$X[0]], 16 + strlen($X[0])) : '');
                } else {
                    unset($X[6]);
                    if ($e != $X[0]) {
                        queries('EXEC sp_rename '.q(table($R).".$e").', '.q(idf_unescape($X[0])).", 'COLUMN'");
                    }
...
false;
        ...
        return(!$v || queries('DROP INDEX '.implode(', ', $v))) && (!$Xb || queries('ALTER TABLE '.table($R).' DROP '.implode(', ', $Xb)));
    }
    function last_id()
    {
        global$g;
    ...
    {
        $J = [];
        foreach (get_rows('EXEC sp_fkeys @fktable_name = '.q($R))as$K) {
            $q = &$J[$K['FK_NAME']];
...
apply_queries('TRUNCATE TABLE', $T);
    ...
    {
        return
queries('DROP VIEW '.implode(', ', array_map('table', $li)));
    }
    function drop_tables($T)
    {
        return
queries('DROP TABLE '.implode(', ', array_map('table', $T)));
    ...
    {
        return
apply_queries('ALTER SCHEMA '.idf_escape($lh).' TRANSFER', array_merge($T, $li));
    }
    function trigger($C)
    {
        if ($C == '') {
...
preg_match('~^(columns|database|drop_col|indexes|scheme|sql|table|trigger|view|view_trigger)$~', $Ec);
    ...
    $Oh = [];
    $Vg = [];
    foreach (['Numbers'=>['tinyint'=>3, 'smallint'=>5, 'int'=>10, 'bigint'=>20, 'bit'=>1, 'decimal'=>0, 'real'=>12, 'float'=>53, 'smallmoney'=>10, 'money'=>20], 'Date and time'=>['date'=>10, 'smalldatetime'=>19, 'datetime'=>19, 'datetime2'=>19, 'time'=>8, 'datetimeoffset'=>10], 'Strings'=>['char'=>8000, 'varchar'=>8000, 'text'=>2147483647, 'nchar'=>4000, 'nvarchar'=>4000, 'ntext'=>1073741823], 'Binary'=>['binary'=>8000, 'varbinary'=>8000, 'image'=>2147483647]]as$y=>$X) {
        $Oh += $X;
        $Vg[$y] = array_keys($X);
    }
    $Vh = [];
    $Ve = ['=', '<', '>', '<=', '>=', '!=', 'LIKE', 'LIKE %%', 'IN', 'IS NULL', 'NOT LIKE', 'NOT IN', 'IS NOT NULL'];
    $Xc = ['len', 'lower', 'round', 'upper'];
    $cd = ['avg', 'count', 'count distinct', 'max', 'min', 'sum'];
    $ec = [['date|time'=>'getdate'], ['int|decimal|real|float|money|datetime'=>'+/-', 'char|text'=>'+']];
}$Wb['firebird'] = 'Firebird (alpha)'; if (isset($_GET['firebird'])) {
    $Ef = ['interbase'];
    ...
    if (extension_loaded('interbase')) {
        class Min_DB
        {
            ...
            public function connect($N, $V, $G)
            {
                ...
                if ($this->_link) {
                    $Yh = explode(':', $N);
                    $this->service_link = ibase_service_attach($Yh[0], $V, $G);
...
ORDER BY r.RDB$FIELD_POSITION';
        $I = ibase_query($g->_link, $H);
        while ($K = ibase_fetch_assoc($I)) {
            $J[trim($K['FIELD_NAME'])] = ['field'=>trim($K['FIELD_NAME']), 'full_type'=>trim($K['FIELD_TYPE']), 'type'=>trim($K['FIELD_SUB_TYPE']), 'default'=>trim($K['FIELD_DEFAULT_VALUE']), 'null'=>(trim($K['FIELD_NOT_NULL_CONSTRAINT']) == 'YES'), 'auto_increment'=>'0', 'collation'=>trim($K['FIELD_COLLATION']), 'privileges'=>['insert'=>1, 'select'=>1, 'update'=>1], 'comment'=>trim($K['FIELD_DESCRIPTION'])];
        }
...
true;
    ...
    {
        return
preg_match('~^(columns|sql|status|table)$~', $Ec);
    }
    $x = 'firebird';
    $Ve = ['='];
    $Xc = [];
    $cd = [];
    $ec = [];
}$Wb['simpledb'] = 'SimpleDB'; if (isset($_GET['simpledb'])) {
    $Ef = ['SimpleXML'];
    ...
    if (class_exists('SimpleXMLElement')) {
        class Min_DB
        {
            ...
            public $server_infovar = '2009-04-15';
            public $errorvar ;
            public $timeoutvar ;
            public $nextvar ;
            public $affected_rowsvar ;
            ...
            public function query($H, $Ph = false)
            {
                $F = ['SelectExpression'=>$H, 'ConsistentRead'=>'true'];
                if ($this->next) {
                ...
                if (preg_match('~^\s*SELECT\s+COUNT\(~i', $H)) {
                    ...
                    foreach ($I
as$Gd) {
                        $Zg += $Gd->Attribute->Value;
                    }
                    $I = [(object) ['Attribute'=>[(object) ['Name'=>'Count', 'Value'=>$Zg]]]];
                }
...
as$Gd) {
                    $K = [];
                    if ($Gd->Name != '') {
                        $K['itemName()'] = (string) $Gd->Name;
                    }
                    foreach ($Gd->Attribute
as$Ha) {
...
as$y=>$X) {
            ...
            public function _processValue($hc)
            {
                return is_object($hc) && $hc['encoding'] == 'base64' ? base64_decode($hc) : (string) $hc;
            }
...
array_values($J);
            ...
            public function fetch_field()
            {
                $Md = array_keys($this->_rows[0]);

                return(object) ['name'=>$Md[$this->_offset++]];
            }
        }
    }
    class Min_Driver extends
Min_SQL
    {
        ...
        public function _chunkRequest($nd, $ua, $F, $xc = [])
        {
            global$g;
            foreach (array_chunk($nd, 25)as$fb) {
                $of = $F;
                foreach ($fb
as$s=>$t) {
                    $of["Item.$s.ItemName"] = $t;
                    foreach ($xc
as$y=>$X) {
                        $of["Item.$s.$y"] = $X;
                    }
                }
                if (!sdb_request($ua, $of)) {
                    return
false;
                }
            }
            $g->affected_rows = count($nd);

            return
true;
        }

        public function _extractIds($R, $Sf, $z)
        {
            $J = [];
            if (preg_match_all("~itemName\(\) = (('[^']*+')+)~", $Sf, $he)) {
                $J = array_map('idf_unescape', $he[1]);
            } else {
                foreach (sdb_request_all('Select', 'Item', ['SelectExpression'=>'SELECT itemName() FROM '.table($R).$Sf.($z ? ' LIMIT 1' : '')])as$Gd) {
                    $J[] = $Gd->Name;
                }
            }
        ...
        public function select($R, $M, $Z, $ad, $af = [], $z = 1, $E = 0, $Jf = false)
        {
            global$g;
            $g->next = $_GET['next'];
            $J = parent::select($R, $M, $Z, $ad, $af, $z, $E, $Jf);
            $g->next = 0;
        ...
        public function delete($R, $Sf, $z = 0)
        {
            return$this->_chunkRequest($this->_extractIds($R, $Sf, $z), 'BatchDeleteAttributes', ['DomainName'=>$R]);
        }
...
as$y=>$X) {
                $y = idf_unescape($y);
                if ($X == 'NULL' || ($t != '' && [$t] != $nd)) {
                    $Lb['Attribute.'.count($Lb).'.Name'] = $y;
                }
                if ($X != 'NULL') {
                    foreach ((array) $X
as$Id=>$W) {
                        $yd["Attribute.$s.Name"] = $y;
                        $yd["Attribute.$s.Value"] = (is_array($X) ? $W : idf_unescape($W));
                        if (!$Id) {
                            $yd["Attribute.$s.Replace"] = 'true';
                        }
                        $s++;
                    }
                }
            ...
            return(!$yd || $this->_chunkRequest(($t != '' ? [$t] : $nd), 'BatchPutAttributes', $F, $yd)) && (!$Lb || $this->_chunkRequest($nd, 'BatchDeleteAttributes', $F, $Lb));
        }
...
[];
    ...
    function table_status($C = '', $Dc = false)
    {
        ...
        foreach (($C != '' ? [$C=>true] : tables_list())as$R=>$U) {
            $K = ['Name'=>$R, 'Auto_increment'=>''];
            if (!$Dc) {
                $ue = sdb_request('DomainMetadata', ['DomainName'=>$R]);
                if ($ue) {
...
[$m=>count(tables_list())];
    ...
    function hmac($Aa, $Eb, $y, $Wf = false)
    {
        $Ta = 64;
        if (strlen($y) > $Ta) {
            $y = pack('H*', $Aa($y));
        ...
        $y = str_pad($y, $Ta, "\0");
        $Jd = $y ^ str_repeat("\x36", $Ta);
        $Kd = $y ^ str_repeat("\x5C", $Ta);
        $J = $Aa($Kd.pack('H*', $Aa($Jd.$Eb)));
        if ($Wf) {
            $J = pack('H*', $J);
        }
    ...
    {
        global$b,$g;
        list($kd, $F['AWSAccessKeyId'], $xg) = $b->credentials();
        $F['Action'] = $ua;
        ...
        foreach ($F
as$y=>$X) {
            $H .= '&'.rawurlencode($y).'='.rawurlencode($X);
        }
        $H = str_replace('%7E', '~', substr($H, 1));
        $H .= '&Signature='.urlencode(base64_encode(hmac('sha1', "POST\n".preg_replace('~^https?://~', '', $kd)."\n/\n$H", $xg, true)));
        @ini_set('track_errors', 1);
        $Hc = @file_get_contents((preg_match('~^https?://~', $kd) ? $kd : "http://$kd"), false, stream_context_create(['http'=>['method'=>'POST', 'content'=>$H, 'ignore_errors'=>1]]));
        if (!$Hc) {
...
false;
        ...
        return $ri->$kh ? $ri->$kh : true;
    }
    function sdb_request_all($ua, $kh, $F = [], $th = 0)
    {
        $J = [];
        $Sg = ($th ? microtime(true) : 0);
...
as$hc) {
            ...
            if ($z && count($J) >= $z) {
                $_GET['next'] = $ri->NextToken;
                break;
            }
            if ($th && microtime(true) - $Sg > $th) {
                return
false;
            }
            $F['NextToken'] = $ri->NextToken;
            if ($z) {
    ...
    $x = 'simpledb';
    $Ve = ['=', '<', '>', '<=', '>=', '!=', 'LIKE', 'LIKE %%', 'IN', 'IS NULL', 'NOT LIKE', 'IS NOT NULL'];
    $Xc = [];
    $cd = ['count'];
    $ec = [['json']];
}$Wb['mongo'] = 'MongoDB (beta)'; if (isset($_GET['mongo'])) {
    $Ef = ['mongo'];
    define('DRIVER', 'mongo');
    if (class_exists('MongoDB')) {
        class Min_DB
        {
            ...
            public function connect($N, $V, $G)
            {
                ...
                try {
                    $this->_link = @new
MongoClient("mongodb://$N", $Ye);

                    return
true;
                } catch (Exception$tc) {
                    $this->error = $tc->getMessage();
...
as$y=>$X) {
                        ...
                        $K[$y] = (is_a($X, 'MongoId') ? 'ObjectId("'.strval($X).'")' : (is_a($X, 'MongoDate') ? gmdate('Y-m-d H:i:s', $X->sec).' GMT' : (is_a($X, 'MongoBinData') ? $X->bin : (is_a($X, 'MongoRegex') ? strval($X) : (is_object($X) ? get_class($X) : $X)))));
                    }
                    $this->_rows[] = $K;
                    foreach ($K
as$y=>$X) {
...
as$X) {
            ...
            return
new
Min_Result($this->_conn->_db->selectCollection($R)->find([], $M)->sort($Mg)->limit(+$z)->skip($E * $z));
        }

        public function insert($R, $O)
        {
            try {
                $J = $this->_conn->_db->selectCollection($R)->insert($O);
                ...
                $this->_conn->error = $tc->getMessage();
...
[];
    ...
    function count_tables($l)
    {
        ...
        foreach ($l
as$m) {
            $J[$m] = count($g->_link->selectDB($m)->getCollectionNames(true));
        }
...
true;
    ...
    function indexes($R, $h = null)
    {
        ...
        foreach ($g->_db->selectCollection($R)->getIndexInfo()as$v) {
            $Ob = [];
            foreach ($v['key']as$e=>$U) {
                $Ob[] = ($U == -1 ? '1' : null);
            }
            $J[$v['name']] = ['type'=>($v['name'] == '_id_' ? 'PRIMARY' : ($v['unique'] ? 'UNIQUE' : 'INDEX')), 'columns'=>array_keys($v['key']), 'lengths'=>[], 'descs'=>$Ob];
        }
...
true;
    ...
    function drop_tables($T)
    {
        ...
        foreach ($T
as$R) {
            $hg = $g->_db->selectCollection($R)->drop();
            if (!$hg['ok']) {
                return
false;
            }
...
true;
    ...
    function truncate_tables($T)
    {
        ...
        foreach ($T
as$R) {
            $hg = $g->_db->selectCollection($R)->remove();
            if (!$hg['ok']) {
                return
false;
            }
...
as$X) {
            list($U, $C, $O) = $X;
            if ($O == 'DROP') {
                $J = $g->_db->command(['deleteIndexes'=>$R, 'index'=>$C]);
            } else {
                $f = [];
                foreach ($O
as$e) {
                ...
                $J = $g->_db->selectCollection($R)->ensureIndex($f, ['unique'=>($U == 'UNIQUE'), 'name'=>$C]);
            }
            if ($J['errmsg']) {
                $g->error = $J['errmsg'];
...
true;
    ...
    {
        return
preg_match('~database|indexes~', $Ec);
    }
    $x = 'mongo';
    $Ve = ['='];
    $Xc = [];
    $cd = [];
    $ec = [['json']];
}$Wb['elastic'] = 'Elasticsearch (beta)'; if (isset($_GET['elastic'])) {
    $Ef = ['json'];
    ...
    if (function_exists('json_decode')) {
        class Min_DB
        {
            ...
            public function rootQuery($wf, $vb = [], $ve = 'GET')
            {
                @ini_set('track_errors', 1);
                $Hc = @file_get_contents($this->_url.'/'.ltrim($wf, '/'), false, stream_context_create(['http'=>['method'=>$ve, 'content'=>json_encode($vb), 'ignore_errors'=>1]]));
                if (!$Hc) {
...
false;
            ...
            public function query($wf, $vb = [], $ve = 'GET')
            {
                return$this->rootQuery(($this->_db != '' ? "$this->_db/" : '/').ltrim($wf, '/'), $vb, $ve);
            }

            public function connect($N, $V, $G)
            {
                preg_match('~^(https?://)?(.*)~', $N, $B);
                $this->_url = ($B[1] ? $B[1] : 'http://')."$V:$G@$B[2]/";
                $J = $this->query('');
                if ($J) {
...
as$kb) {
            ...
            foreach ($Z
as$X) {
                list($kb, $Te, $X) = explode(' ', $X, 3);
                if ($kb == '_id') {
                    $Eb['query']['ids']['values'][] = $X;
                } elseif ($kb.$X != '') {
                    $oh = ['term'=>[($kb != '' ? $kb : '_all')=>$X]];
                    if ($Te == '=') {
                        $Eb['query']['filtered']['filter']['and'][] = $oh;
                    } else {
                        $Eb['query']['filtered']['query']['bool']['must'][] = $oh;
                    }
                }
            }
            if ($Eb['query'] && !$Eb['query']['filtered']['query'] && !$Eb['query']['ids']) {
                $Eb['query']['filtered']['query'] = ['match_all'=>[]];
            }
            $Sg = microtime(true);
            $wg = $this->_conn->query($H, $Eb);
            if ($Jf) {
                echo$b->selectQuery("$H: ".print_r($Eb, true), format_time($Sg));
            }
            if (!$wg) {
                return
false;
...
as$y=>$X) {
                    ...
                    $K[$y] = (is_array($X) ? json_encode($X) : $X);
                }
                $J[] = $K;
            }
...
Min_DB();
    ...
    {
        return
preg_match('~database|table|columns~', $Ec);
    }
    function logged_user()
    {
        global$b;
...
[];
    ...
    {
        global$g;
        $wg = $g->query('_search?search_type=count', ['facets'=>['count_by_type'=>['terms'=>['field'=>'_type']]]], 'POST');
        $J = [];
        if ($wg) {
            foreach ($wg['facets']['count_by_type']['terms']as$R) {
                $J[$R['term']] = ['Name'=>$R['term'], 'Engine'=>'table', 'Rows'=>$R['count']];
                if ($C != '' && $C == $R['term']) {
                    return$J[$C];
                }
            }
...
[['type'=>'PRIMARY', 'columns'=>['_id']]];
    ...
    {
        global$g;
        $I = $g->query("$R/_mapping");
        $J = [];
        if ($I) {
            $ee = $I[$R]['properties'];
            if (!$ee) {
                $ee = $I[$g->_db]['mappings'][$R]['properties'];
            }
            if ($ee) {
                foreach ($ee
as$C=>$o) {
                    $J[$C] = ['field'=>$C, 'full_type'=>$o['type'], 'type'=>$o['type'], 'privileges'=>['insert'=>1, 'select'=>1, 'update'=>1]];
                    if ($o['properties']) {
...
[];
    ...
    function drop_tables($T)
    {
        ...
        foreach ($T
as$R) {
            $J = $J && $g->query(urlencode($R), [], 'DELETE');
        }
    ...
    $cd = [];
    $ec = [['json']];
}$Wb = ['server'=>'MySQL'] + $Wb; if (!defined('DRIVER')) {
    $Ef = ['MySQLi', 'MySQL', 'PDO_MySQL'];
...
MySQLi
        {
            ...
            public function connect($N = '', $V = '', $G = '', $k = null, $Af = null, $Lg = null)
            {
...
as$m) {
    ...
    function fk_support($S)
    {
        ...
        return
preg_match('~InnoDB|IBMDB2I~i', $S['Engine']) || (preg_match('~NDB~i', $S['Engine']) && version_compare($g->server_info, '5.6') >= 0);
    }
    function fields($R)
    {
        $J = [];
        foreach (get_rows('SHOW FULL COLUMNS FROM '.table($R))as$K) {
            ...
            $J[$K['Field']] = ['field'=>$K['Field'], 'full_type'=>$K['Type'], 'type'=>$B[1], 'length'=>$B[2], 'unsigned'=>ltrim($B[3].$B[4]), 'default'=>($K['Default'] != '' || preg_match('~char|set~', $B[1]) ? $K['Default'] : null), 'null'=>($K['Null'] == 'YES'), 'auto_increment'=>($K['Extra'] == 'auto_increment'), 'on_update'=>(preg_match('~^on update (.+)~i', $K['Extra'], $B) ? $B[1] : ''), 'collation'=>$K['Collation'], 'privileges'=>array_flip(preg_split('~, *~', $K['Privileges'])), 'comment'=>$K['Comment'], 'primary'=>($K['Key'] == 'PRI')];
        }
...
as$B) {
                ...
                $J[idf_unescape($B[1])] = ['db'=>idf_unescape($B[4] != '' ? $B[3] : $B[4]), 'table'=>idf_unescape($B[4] != '' ? $B[4] : $B[3]), 'source'=>array_map('idf_unescape', $Ng[0]), 'target'=>array_map('idf_unescape', $lh[0]), 'on_delete'=>($B[6] ? $B[6] : 'RESTRICT'), 'on_update'=>($B[7] ? $B[7] : 'RESTRICT')];
            }
        }
...
queries('DROP TABLE '.implode(', ', array_map('table', $T)));
    ...
    function move_tables($T, $li, $lh)
    {
        ...
        return
queries('RENAME TABLE '.implode(', ', $fg));
    }
...
['Timing'=>['BEFORE', 'AFTER'], 'Event'=>['INSERT', 'UPDATE', 'DELETE'], 'Type'=>['FOR EACH ROW']];
    ...
    {
        global$g,$oc,$wd,$Oh;
        $Ba = ['bool', 'boolean', 'integer', 'double precision', 'real', 'dec', 'numeric', 'fixed', 'national char', 'national varchar'];
...
as$nf) {
            ...
            $p[] = ['field'=>$C, 'type'=>strtolower($nf[5]), 'length'=>preg_replace_callback("~$oc~s", 'normalize_enum', $nf[6]), 'unsigned'=>strtolower(preg_replace('~\\s+~', ' ', trim("$nf[8] $nf[7]"))), 'null'=>1, 'full_type'=>$nf[4], 'inout'=>strtoupper($nf[1]), 'collation'=>strtolower($nf[9])];
        }
        if ($U != 'FUNCTION') {
            return
...
queries('KILL '.number($X));
    ...
    $Vh = ['unsigned', 'zerofill', 'unsigned zerofill'];
    $Ve = ['=', '<', '>', '<=', '>=', '!=', 'LIKE', 'LIKE %%', 'REGEXP', 'IN', 'IS NULL', 'NOT LIKE', 'NOT REGEXP', 'NOT IN', 'IS NOT NULL', 'SQL'];
    $Xc = ['char_length', 'date', 'from_unixtime', 'lower', 'round', 'sec_to_time', 'time_to_sec', 'upper'];
    $cd = ['avg', 'count', 'count distinct', 'group_concat', 'max', 'min', 'sum'];
    $ec = [['char'=>'md5/sha1/password/encrypt/uuid', 'binary'=>'md5/sha1', 'date|time'=>'now'], ['(^|[^o])int|float|double|decimal'=>'+/-', 'date'=>'+ interval/- interval', 'time'=>'addtime/subtime', 'char|text'=>'concat']];
}define('SERVER', $_GET[DRIVER]); define('DB', $_GET['db']); define('ME', preg_replace('~^[^?]*/([^?]*).*~', '\\1', $_SERVER['REQUEST_URI']).'?'.(sid() ? SID.'&' : '').(SERVER !== null ? DRIVER.'='.urlencode(SERVER).'&' : '').(isset($_GET['username']) ? 'username='.urlencode($_GET['username']).'&' : '').(DB != '' ? 'db='.urlencode(DB).'&'.(isset($_GET['ns']) ? 'ns='.urlencode($_GET['ns']).'&' : '') : '')); $ia = '4.3.0'; class Adminer
{
    public $operators;
...
DB;
    ...
    public function databases($Oc = true)
    {
        return
get_databases($Oc);
    }
...
',"<p><input type='submit' value='".'Login'."'>\n",checkbox('auth[permanent]', 1, $_COOKIE['adminer_permanent'], 'Permanent login')."\n";
    ...
    public function login($ce, $G)
    {
        ...
        if ($x == 'sqlite') {
            return
sprintf('<a href="https://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to use SQLite.', '<code>login()</code>');
        }
...
true;
    ...
    public function tableName($ch)
    {
        return
h($ch['Name']);
    }

    public function fieldName($o, $af = 0)
    {
        return'<span title="'.h($o['full_type']).'">'.h($o['field']).'</span>';
    }

    public function selectLinks($ch, $O = '')
    {
        echo'<p class="links">';
        $be = ['select'=>'Select data'];
...
foreign_keys($R);
    ...
    public function backwardKeys($R, $bh)
    {
        return
[];
    }

    public function backwardKeysPrint($Na, $K)
    {
    }

    public function selectQuery($H, $sh)
    {
        global$x;

        return"<p><code class='jush-$x'>".h(str_replace("\n", ' ', $H))."</code> <span class='time'>($sh)</span>".(support('sql') ? " <a href='".h(ME).'sql='.urlencode($H)."'>".'Edit'.'</a>' : '').'</p>';
    }
...
shorten_utf8(trim($H), 1000);
    ...
    public function rowDescriptions($L, $Qc)
    {
        return$L;
    }
    ...
    public function selectVal($X, $_, $o, $if)
    {
        ...
        if (preg_match('~blob|bytea|raw|file~', $o['type']) && !is_utf8($X)) {
            $J = '<i>'.lang(['%d byte', '%d bytes'], strlen($if)).'</i>';
        }
        if (preg_match('~json~', $o['type'])) {
            $J = "<code class='jush-js'>$J</code>";
        }
    ...
    public function tableStructurePrint($p)
    {
        ...
        foreach ($p
as$o) {
            echo'<tr'.odd().'><th>'.h($o['field']),"<td><span title='".h($o['collation'])."'>".h($o['full_type']).'</span>',($o['null'] ? ' <i>NULL</i>' : ''),($o['auto_increment'] ? ' <i>'.'Auto Increment'.'</i>' : ''),(isset($o['default']) ? " <span title='".'Default value'."'>[<b>".h($o['default']).'</b>]</span>' : ''),(support('comment') ? '<td>'.nbsp($o['comment']) : ''),"\n";
        }
        echo"</table>\n";
    }
...
as$C=>$v) {
            ...
            echo"<tr title='".h($C)."'><th>$v[type]<td>".implode(', ', $Jf)."\n";
        }
        echo"</table>\n";
    }

    public function selectColumnsPrint($M, $f)
    {
        global$Xc,$cd;
...
as$y=>$X) {
            ...
            echo'<div>'.($Xc || $cd ? "<select name='columns[$s][fun]' onchange='helpClose();".($y !== '' ? '' : ' this.nextSibling.nextSibling.onchange();')."'".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'", 1).'>'.optionlist([-1=>''] + array_filter(['Functions'=>$Xc, 'Aggregation'=>$cd]), $X['fun']).'</select>'."($e)" : $e)."</div>\n";
            $s++;
        }
        echo"</div></fieldset>\n";
    }

    public function selectSearchPrint($Z, $f, $w)
    {
        print_fieldset('search', 'Search', $Z);
        foreach ($w
as$s=> $v) {
            if ($v['type'] == 'FULLTEXT') {
                echo'(<i>'.implode('</i>, <i>', array_map('h', $v['columns'])).'</i>) AGAINST'," <input type='search' name='fulltext[$s]' value='".h($_GET['fulltext'][$s])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$s]", 1, isset($_GET['boolean'][$s]), 'BOOL'),"<br>\n";
            }
        }
        ...
        for ($s = 0; $s <= count($_GET['where']); $s++) {
            ...
            if (!$X || ("$X[col]$X[val]" != '' && in_array($X['op'], $this->operators))) {
                echo'<div>'.select_input(" name='where[$s][col]' onchange='$Ya'", $f, $X['col'], '('.'anywhere'.')'),html_select("where[$s][op]", $this->operators, $X['op'], $Ya),"<input type='search' name='where[$s][val]' value='".h($X['val'])."' onchange='".($X ? 'selectFieldChange(this.form)' : 'selectAddRow(this)').";' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";
            }
        }
        echo"</div></fieldset>\n";
    }

    public function selectOrderPrint($af, $f, $w)
    {
    ...
    public function selectLengthPrint($rh)
    {
        if ($rh !== null) {
...
as$y=>$X) {
    ...
    public function selectEmailPrint($jc, $f)
    {
    }

    public function selectColumnsProcess($f, $w)
    {
        global$Xc,$cd;
        $M = [];
        $ad = [];
...
as$C=>$o) {
    ...
    public function selectEmailProcess($Z, $Qc)
    {
        return
false;
    }

    public function selectQueryBuild($M, $Z, $ad, $af, $z, $E)
    {
        return'';
    }

    public function messageQuery($H, $sh)
    {
        global$x;
        restart_session();
        ...
        $hd[$_GET['db']][] = [$H, time(), $sh];

        return" <span class='time'>".@date('H:i:s')."</span> <a href='#$t' onclick=\"return !toggle('$t');\">".'SQL command'.'</a>'."<div id='$t' class='hidden'><pre><code class='jush-$x'>".shorten_utf8($H, 1000).'</code></pre>'.($sh ? " <span class='time'>($sh)</span>" : '').(support('sql') ? '<p><a href="'.h(str_replace('db='.urlencode(DB), 'db='.urlencode($_GET['db']), ME).'sql=&history='.(count($hd[$_GET['db']]) - 1)).'">'.'Edit'.'</a>' : '').'</div>';
    }
...
explode('/', $J);
    ...
    public function editInput($R, $o, $Ia, $Y)
    {
        if ($o['type'] == 'enum') {
...
['sql'=>'SQL', 'csv'=>'CSV,', 'csv;'=>'CSV;', 'tsv'=>'TSV'];
    ...
    public function dumpDatabase($m)
    {
    }

    public function dumpTable($R, $Wg, $Fd = 0)
    {
        if ($_POST['format'] != 'sql') {
            echo"\xef\xbb\xbf";
            if ($Wg) {
    ...
    public function dumpData($R, $Wg, $H)
    {
        global$g,$x;
...
as$y=>$X) {
    ...
    public function dumpFilename($md)
    {
        return
friendly_url($md != '' ? $md : (SERVER != '' ? SERVER : 'localhost'));
    }

    public function dumpHeaders($md, $ye = false)
    {
        $lf = $_POST['output'];
...
as$V=>$G) {
            ...
            echo'<script type="text/javascript" src="',h(preg_replace('~\\?.*~', '', ME)).'?file=jush.js&amp;version=4.3.0','"></script>
';
            if (support('sql')) {
                echo'<script type="text/javascript">
';
...
';
    ...
    public function databasesPrint($xe)
    {
        global$b,$g;
        $l = $this->databases();
        echo'<form action="">
...
';
        ...
        echo"<span title='".'database'."'>DB</span>: ".($l ? "<select name='db'$Gb>".optionlist([''=>''] + $l, DB).'</select>' : '<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".'Use'."'".($l ? " class='hidden'" : '').">\n";
        if ($xe != 'db' && DB != '' && $g->select_db(DB)) {
            if (support('scheme')) {
                echo'<br>'.'Schema'.": <select name='ns'$Gb>".optionlist([''=>''] + $b->schemas(), $_GET['ns']).'</select>';
                if ($_GET['ns'] != '') {
                    set_schema($_GET['ns']);
                }
            }
...
as$R=>$P) {
            ...
            $C = $this->tableName($P);
            echo(support('table') || support('indexes') ? '<a href="'.h(ME).'table='.urlencode($R).'"'.bold(in_array($R, [$_GET['table'], $_GET['create'], $_GET['indexes'], $_GET['foreign'], $_GET['trigger']]), (is_view($P) ? 'view' : 'structure'))." title='".'Show structure'."'>$C</a>" : "<span>$C</span>")."\n";
        }
        echo"</ul>\n";
    }
...
Adminer()); if ($b->operators === null) {
    $b->operators = $Ve;
} function page_header($vh, $n = '', $Va = [], $wh = '')
{
    global$ca,$ia,$b,$Wb,$x;
    page_headers();
    if (is_ajax() && $n) {
...
';
    if ($b->head()) {
        echo'<link rel="shortcut icon" type="image/x-icon" href="',h(preg_replace('~\\?.*~', '', ME)).'?file=favicon.ico&amp;version=4.3.0','">
...
';
    if ($Va !== null) {
        ...
        echo'<p id="breadcrumb"><a href="'.h($_ ? $_ : '.').'">'.$Wb[DRIVER].'</a> &raquo; ';
...
as$y=>$X) {
                    ...
                    if ($Nb != '') {
                        echo"<a href='".h(ME."$y=").urlencode(is_array($X) ? $X[0] : $X)."'>$Nb</a> &raquo; ";
                    }
                }
            }
            echo"$vh\n";
...
{
    return
int32((($ti >> 5 & 0x7FFFFFF) ^ $si << 2) + (($si >> 3 & 0x1FFFFFFF) ^ $ti << 4)) ^ int32(($Zg ^ $si) + ($Id ^ $ti));
...
false;
    ...
    return
long2str($W, true);
}$g = ''; $gd = $_SESSION['token']; if (!$gd) {
    $_SESSION['token'] = rand(1, 1e6);
} $Ah = get_token(); $zf = []; if ($_COOKIE['adminer_permanent']) {
...
as$Bd=>$X) {
    ...
    if ($Fe > 0) {
        auth_error(lang(['Too many unsuccessful logins, try again in %d minute.', 'Too many unsuccessful logins, try again in %d minutes.'], ceil($Fe / 60)));
    }
...
}function auth_error($n)
{
    ...
    if (isset($_GET['username'])) {
        ...
        if (($_COOKIE[$Gg] || $_GET[$Gg]) && !$gd) {
            ...
            if ($G !== null) {
                if ($G === false) {
                    $n .= '<br>'.sprintf('Master password expired. <a href="https://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to make it permanent.', '<code>permanentLogin()</code>');
                }
...
Min_Driver($g); if (!is_object($g) || ($ce = $b->login($_GET['username'], get_password())) !== true) {
    ...
    session_write_close();
} function select($I, $h = null, $df = [], $z = 0)
{
    global$x;
    $be = [];
    $w = [];
    ...
    for ($s = 0; (!$z || $s < $z) && ($K = $I->fetch_row()); $s++) {
        if (!$s) {
            ...
            for ($Hd = 0; $Hd < count($K); $Hd++) {
                ...
                $Oh[$Hd] = $o->type;
                echo'<th'.($cf != '' || $o->name != $bf ? " title='".h(($cf != '' ? "$cf." : '').$bf)."'" : '').'>'.h($C).($df ? doc_link(['sql'=>'explain-output.html#explain_'.strtolower($C)]) : '');
            }
            echo"</thead>\n";
        }
        echo'<tr'.odd().'>';
...
<td><input name="',h($y),'[length]" value="',h($o['length']),'" size="3" onfocus="editingLengthFocus(this);"',(!$o['length'] && preg_match('~var(char|binary)$~', $U) ? " class='required'" : ''),' onchange="editingLengthChange(this);" onkeyup="this.onchange();" aria-labelledby="label-length"><td class="options">';
    echo"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~', $U) ? '' : " class='hidden'").'><option value="">('.'collation'.')'.optionlist($mb, $o['collation']).'</select>',($Vh ? "<select name='".h($y)."[unsigned]'".(!$U || preg_match('~((^|[^o])int|float|double|decimal)$~', $U) ? '' : " class='hidden'").'><option>'.optionlist($Vh, $o['unsigned']).'</select>' : ''),(isset($o['on_update']) ? "<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~', $U) ? '' : " class='hidden'").'>'.optionlist([''=>'('.'ON UPDATE'.')', 'CURRENT_TIMESTAMP'], $o['on_update']).'</select>' : ''),($Rc ? "<select name='".h($y)."[on_delete]'".(preg_match('~`~', $U) ? '' : " class='hidden'")."><option value=''>(".'ON DELETE'.')'.optionlist(explode('|', $Qe), $o['on_delete']).'</select> ' : ' ');
...
}function edit_fields($p, $mb, $U = 'TABLE', $Rc = [], $rb = false)
{
    global$g,$wd;
    $p = array_values($p);
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="Auto Increment">AI</acronym>',doc_link(['sql'=>'example-auto-increment.html', 'sqlite'=>'autoinc.html', 'pgsql'=>'datatype.html#DATATYPE-SERIAL', 'mssql'=>'ms186775.aspx']),'<td id="label-default">Default value
',(support('comment') ? "<td id='label-comment'".($rb ? '' : " class='hidden'").'>'.'Comment' : '');
    }
    echo'<td>',"<input type='image' class='icon' name='add[".(support('move_col') ? 0 : count($p))."]' src='".h(preg_replace('~\\?.*~', '', ME))."?file=plus.gif&amp;version=4.3.0' alt='+' title='".'Add next'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
as$s=>$o) {
        ...
        echo'<tr',($Rb ? '' : " style='display: none;'"),'>
',($U == 'PROCEDURE' ? '<td>'.html_select("fields[$s][inout]", explode('|', $wd), $o['inout']) : ''),'<th>';
        if ($Rb) {
            echo'<input name="fields[',$s,'][field]" value="',h($o['field']),'" onchange="editingNameChange(this);',($o['field'] != '' || count($p) > 1 ? '' : ' editingAddRow(this);" onkeyup="if (this.value) editingAddRow(this);'),'" maxlength="64" autocapitalize="off" aria-labelledby="label-name">';
        }
...
',(support('comment') ? '<td'.($rb ? '' : " class='hidden'")."><input name='fields[$s][comment]' value='".h($o['comment'])."' maxlength='".($g->server_info >= 5.5 ? 1024 : 255)."' aria-labelledby='label-comment'>" : '');
        }
        echo'<td>',(support('move_col') ? "<input type='image' class='icon' name='add[$s]' src='".h(preg_replace('~\\?.*~', '', ME))."?file=plus.gif&amp;version=4.3.0' alt='+' title='".'Add next'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace('~\\?.*~', '', ME))."?file=up.gif&amp;version=4.3.0' alt='^' title='".'Move up'."' onclick='return !editingMoveRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace('~\\?.*~', '', ME))."?file=down.gif&amp;version=4.3.0' alt='v' title='".'Move down'."' onclick='return !editingMoveRow(this, 0);'>&nbsp;" : ''),($ef == '' || support('drop_col') ? "<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace('~\\?.*~', '', ME))."?file=cross.gif&amp;version=4.3.0' alt='x' title='".'Remove'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">" : ''),"\n";
    }
...
}function tar_file($Ic, $zh)
{
    ...
    $zh->send();
    echo
str_repeat("\0", 511 - ($zh->size + 511) % 512);
}function ini_bytes($vd)
{
    $X = ini_get($vd);
    switch (strtolower(substr($X, -1))) {case'g':$X *= 1024; case'm':$X *= 1024; case'k':$X *= 1024; }

    return$X;
}function doc_link($xf)
{
    global$x,$g;
    $Zh = ['sql'=>'http://dev.mysql.com/doc/refman/'.substr($g->server_info, 0, 3).'/en/', 'sqlite'=>'http://www.sqlite.org/', 'pgsql'=>'http://www.postgresql.org/docs/'.substr($g->server_info, 0, 3).'/static/', 'mssql'=>'http://msdn.microsoft.com/library/', 'oracle'=>'http://download.oracle.com/docs/cd/B19306_01/server.102/b14200/'];
...
}function connect_error()
{
    ...
    if (DB != '') {
        ...
        echo"<p class='links'>\n";
        foreach (['database'=>'Create database', 'privileges'=>'Privileges', 'processlist'=>'Process list', 'variables'=>'Variables', 'status'=>'Status', 'replication'=>'Replication']as$y=>$X) {
            if (support($y)) {
...
as$m=>$T) {
                ...
                $t = h('Db-'.$m);
                echo'<tr'.odd().'>'.(support('database') ? '<td>'.checkbox('db[]', $m, in_array($m, (array) $_POST['db']), '', '', '', $t) : ''),"<th><a href='$ng' id='$m'>".h($m).'</a>';
...
}$Qe = 'RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT'; class TmpFile
{
    public $handler;
    public $size;
...
}$oc = "'(?:''|[^'\\\\]|\\\\.)*'"; $wd = 'IN|OUT|INOUT'; if (isset($_GET['select']) && ($_POST['edit'] || $_POST['clone']) && !$_POST['save']) {
    ...
    $I = $Vb->select($a, $M, [where($_GET, $p)], $M);
    $K = ($I ? $I->fetch_row() : []);
    echo$K[0];
    exit;
    ...
    foreach (table_status('', true)as$R=>$S) {
        ...
        foreach ($b->foreignKeys($R)as$X) {
            if (!$X['db']) {
                ...
                $ug[$R]['references'][$X['table']][(string) $Ud] = [$X['source'], $X['target']];
...
as$Ud=>$f) {
                ...
                foreach ($f
as$lh) {
                    echo"\n<div class='references' title='".h($mh)."' id='refd$Ud-".($s++)."' style='left: $Vd".'em; top: '.$R['fields'][$lh]['pos'].'em; height: 1.25em; background: url('.h(preg_replace('~\\?.*~', '', ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.3.0'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$Vd)."em;'></div></div>";
                }
            }
        }
        echo"\n</div>\n";
...
use_sql($m).";\n\n";
                    ...
                    $kf = '';
                    if ($_POST['routines']) {
                        foreach (['FUNCTION', 'PROCEDURE']as$og) {
...
<table cellspacing="0">
';
    $Hb = ['', 'USE', 'DROP+CREATE', 'CREATE'];
    $hh = ['', 'DROP+CREATE', 'CREATE'];
    $Fb = ['', 'TRUNCATE+INSERT', 'INSERT'];
    if ($x == 'sql') {
        $Fb[] = 'INSERT+UPDATE';
    }
    ...
    echo'<tr><th>'.'Output'.'<td>'.html_select('output', $b->dumpOutput(), $K['output'], 0)."\n";
                ...
                if (!$D && preg_match("~^$Og*+DELIMITER\\s+(\\S+)~i", $H, $B)) {
                    ...
                    if (!$Tc && $Vc && !feof($Vc)) {
                        ...
                        if ($Tc && rtrim($Tc) != $Mb) {
                            ...
                            $Qf = substr($H, 0, $Bf);
                            $pb++;
                            $Jf = "<pre id='sql-$pb'><code class='jush-$x'>".$b->sqlCommandQuery($Qf)."</code></pre>\n";
...
2;
                                        ...
                                        if (!$_POST['only_errors']) {
                                            echo"<form action='' method='post'>\n";
                                            $Ge = $I->num_rows;
                                            echo'<p>'.($Ge ? ($z && $Ge > $z ? sprintf('%d / ', $z) : '').lang(['%d row', '%d rows'], $Ge) : ''),$sh;
                                            $t = "export-$pb";
                                        ...
                                        if (!$_POST['only_errors']) {
                                            echo"<p class='message' title='".h($g->info)."'>".lang(['Query executed OK, %d row affected.', 'Query executed OK, %d rows affected.'], $g->affected_rows)."$sh\n";
                                        }
                                    }
                                    $Sg = microtime(true);
                                } while ($g->next_result());
            ...
            if ($lc) {
                ...
                echo"<p class='message'>".lang(['%d query executed OK.', '%d queries executed OK.'], $pb - count($qc))," <span class='time'>(".format_time($Ch).")</span>\n";
            } elseif ($qc && $pb > 1) {
...
checkbox('error_stops', 1, ($_POST ? $_POST['error_stops'] : isset($_GET['import'])), 'Stop on error')."\n",checkbox('only_errors', 1, ($_POST ? $_POST['only_errors'] : isset($_GET['import'])), 'Show only errors')."\n","<input type='hidden' name='token' value='$Ah'>\n";
    if (!isset($_GET['import']) && $hd) {
        ...
        for ($X = end($hd); $X; $X = prev($hd)) {
            $y = key($hd);
            list($Qf, $sh, $gc) = $X;
            echo'<a href="'.h(ME."sql=&history=$y").'">'.'Edit'.'</a>'." <span class='time' title='".@date('Y-m-d', $sh)."'>".@date('H:i:s', $sh).'</span>'." <code class='jush-$x'>".shorten_utf8(ltrim(str_replace("\n", ' ', str_replace("\r", '', preg_replace('~^(#|-- ).*~m', '', $Qf)))), 80, '</code>').($gc ? " <span class='time'>($gc)</span>" : '')."<br>\n";
        }
...
as$C=>$o) {
        ...
        if ($M) {
            $I = $Vb->select($a, $M, [$Z], $M, [], (isset($_GET['select']) ? 2 : 1));
            $K = $I->fetch_assoc();
            if (!$K) {
                $K = false;
            }
...
as$y=>$X) {
                ...
                $p[$y] = ['field'=>$y, 'null'=>($y != $Vb->primary), 'auto_increment'=>($y == $Vb->primary)];
            }
        }
    }
    edit_form($a, $p, $K, $Wh);
...
as$dh=>$o) {
        ...
        if ($_POST['drop']) {
            ...
            foreach ($K['fields']as$y=>$o) {
                ...
                if ($o['field'] != '') {
                    ...
                    if ($q !== null) {
                        $Pc[idf_escape($o['field'])] = ($a != '' && $x != 'sqlite' ? 'ADD' : ' ').format_foreign_key(['table'=>$Rc[$o['type']], 'source'=>[$o['field']], 'target'=>[$Mh['field']], 'on_delete'=>$o['on_delete']]);
                    }
    ...
    page_header(($a != '' ? 'Alter table' : 'Create table'), $n, ['table'=>$a], h($a));
    if (!$_POST) {
        $K = ['Engine'=>$_COOKIE['adminer_engine'], 'fields'=>[['field'=>'', 'type'=>(isset($Oh['int']) ? 'int' : (isset($Oh['integer']) ? 'integer' : ''))]], 'partition_names'=>['']];
        if ($a != '') {
            $K = $S;
            $K['name'] = $a;
            $K['fields'] = [];
...
';
    if (support('columns') || $a == '') {
        echo'Table name: <input name="name" maxlength="64" value="',h($K['name']),'" autocapitalize="off">
';
        if ($a == '' && !$_POST) {?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }
        echo($nc ? "<select name='Engine' onchange='helpClose();'".on_help('getTarget(event).value', 1).'>'.optionlist([''=>'('.'engine'.')'] + $nc, $K['Engine']).'</select>' : ''),' ',($mb && !preg_match('~sqlite|mssql~', $x) ? html_select('Collation', [''=>'('.'collation'.')'] + $mb, $K['Collation']) : ''),' <input type="submit" value="Save">
';
    }
    echo'
';
...
',checkbox('defaults', 1, true, 'Default values', 'columnShow(this.checked, 5)', 'jsonly');
        ...
        echo(support('comment') ? "<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($rb ? ' checked' : '').'>'.'Comment'.'</label>'.' <input name="Comment" id="Comment" value="'.h($K['Comment']).'" maxlength="'.($g->server_info >= 5.5 ? 2048 : 60).'"'.($rb ? '' : ' class="hidden"').'>' : ''),'<p>
...
';
    ...
    if (support('partitioning')) {
        ...
        echo'<p>
',"<select name='partition_by' onchange='partitionByChange(this);'".on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')", 1).'>'.optionlist([''=>''] + $rf, $K['partition_by']).'</select>','(<input name="partition" value="',h($K['partition']),'">)
...
';
        foreach ($K['partition_names']as$y=>$X) {
            echo'<tr>','<td><input name="partition_names[]" value="'.h($X).'"'.($y == count($K['partition_names']) - 1 ? ' onchange="partitionNameChange(this);"' : '').' autocapitalize="off">','<td><input name="partition_values[]" value="'.h($K['partition_values'][$y]).'">';
        }
        echo'</table>
</div></fieldset>
';
...
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.3.0' alt='+' title='Add next'></noscript>&nbsp;
</thead>
<?php
if ($Hf) {
...
select_input(' disabled', $p, $e),"<label><input disabled type='checkbox'>".'descending'.'</label> ';
    ...
    foreach ($K['indexes']as$v) {
        if (!$_POST['drop_col'] || $Hd != key($_POST['drop_col'])) {
            echo'<tr><td>'.html_select("indexes[$Hd][type]", [-1=>''] + $rd, $v['type'], ($Hd == count($K['indexes']) ? 'indexesAddRow(this);' : 1), 'label-type'),'<td>';
            ksort($v['columns']);
            $s = 1;
            foreach ($v['columns']as$y=>$e) {
            ...
            echo"<td><input name='indexes[$Hd][name]' value='".h($v['name'])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$Hd]' src='".h(preg_replace('~\\?.*~', '', ME))."?file=cross.gif&amp;version=4.3.0' alt='x' title='".'Remove'."' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";
        }
        $Hd++;
    }
    echo'</table>
...
<form action="" method="post">
<p>
',($_POST['add_x'] || strpos($C, "\n") ? '<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>' : '<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($mb ? html_select('collation', [''=>'('.'collation'.')'] + $mb, $K['collation']).doc_link(['sql'=>'charset-charsets.html', 'mssql'=>'ms187963.aspx']) : ''); ?>
...
if (DB != '') {
        ...
        echo"<input type='image' class='icon' name='add' src='".h(preg_replace('~\\?.*~', '', ME))."?file=plus.gif&amp;version=4.3.0' alt='+' title='".'Add next'."'>\n";
    }
    echo'<input type="hidden" name="token" value="',$Ah,'">
</form>
';
...
';
    ...
    if (!$n && $_POST) {
        ...
        echo"<p><code class='jush-$x'>".h($H)."</code> <a href='".h(ME).'sql='.urlencode($H)."'>".'Edit'."</a>\n";
        if (!$g->multi_query($H)) {
            echo"<p class='error'>".error()."\n";
        } else {
            $h = connect();
            ...
            do {
                ...
                if (is_object($I)) {
                    ...
                    echo"<p class='message'>".lang(['Routine has been called, %d row affected.', 'Routine has been called, %d rows affected.'], $g->affected_rows)."\n";
                }
            } while ($g->next_result());
            if ($kf) {
...
';
    ...
    if ($_POST && !$n && !$_POST['add'] && !$_POST['change'] && !$_POST['change-js']) {
        ...
        if ($x == 'sqlite') {
            queries_redirect($A, $pe, recreate_table($a, $a, [], [], [" $C"=>($_POST['drop'] ? '' : ' '.format_foreign_key($K))]));
        } else {
...
';
        $Hd = 0;
        foreach ($K['source']as$y=>$X) {
            echo'<tr>','<td>'.html_select('source['.(+$y).']', [-1=>''] + $Ng, $X, ($Hd == count($K['source']) - 1 ? 'foreignAddRow(this);' : 1), 'label-source'),'<td>'.html_select('target['.(+$y).']', $lh, $K['target'][$y], 1, 'label-target');
            $Hd++;
        }
        echo'</table>
<p>
ON DELETE: ',html_select('on_delete', [-1=>''] + explode('|', $Qe), $K['on_delete']),' ON UPDATE: ',html_select('on_update', [-1=>''] + explode('|', $Qe), $K['on_update']),doc_link(['sql'=>'innodb-foreign-key-constraints.html', 'pgsql'=>'sql-createtable.html#SQL-CREATETABLE-REFERENCES', 'mssql'=>'ms174979.aspx', 'oracle'=>'clauses002.htm#sthref2903']),'<p>
<input type="submit" value="Save">
...
';
    ...
    page_header(($a != '' ? 'Alter view' : 'Create view'), $n, ['table'=>$a], h($a));
    echo'
<form action="" method="post">
...
';
    ...
    $aa = $_GET['event'];
    $zd = ['YEAR', 'QUARTER', 'MONTH', 'DAY', 'HOUR', 'MINUTE', 'WEEK', 'SECOND', 'YEAR_MONTH', 'DAY_HOUR', 'DAY_MINUTE', 'DAY_SECOND', 'HOUR_MINUTE', 'HOUR_SECOND', 'MINUTE_SECOND'];
    $Tg = ['ENABLED'=>'ENABLE', 'DISABLED'=>'DISABLE', 'SLAVESIDE_DISABLED'=>'DISABLE ON SLAVE'];
    $K = $_POST;
    if ($_POST && !$n) {
        if ($_POST['drop']) {
...
';
    ...
    if ($_POST && !process_fields($K['fields']) && !$n) {
        ...
        drop_create("DROP $og ".idf_escape($da), create_routine($og, $K), "DROP $og ".idf_escape($K['name']), create_routine($og, ['name'=>$nh] + $K), "DROP $og ".idf_escape($nh), substr(ME, 0, -1), 'Routine has been dropped.', 'Routine has been altered.', 'Routine has been created.', $da, $K['name']);
    }
...
';
    ...
    $C = $_GET['name'];
    $Kh = trigger_options();
    $K = (array) trigger($C) + ['Trigger'=>$a.'_bi'];
    if ($_POST) {
        if (!$n && in_array($_POST['Timing'], $Kh['Timing']) && in_array($_POST['Event'], $Kh['Event']) && in_array($_POST['Type'], $Kh['Type'])) {
            ...
            if ($_POST['drop']) {
                ...
                if ($C != '') {
                    queries(create_trigger($Pe, $K + ['Type'=>reset($Kh['Type'])]));
                }
            }
        }
        $K = $_POST;
    }
    page_header(($C != '' ? 'Alter trigger'.': '.h($C) : 'Create trigger'), $n, ['table'=>$a]);
    echo'
<form action="" method="post" id="form">
<table cellspacing="0">
...
<p><?php textarea('Statement', $K['Statement']);
    echo'<p>
<input type="submit" value="Save">
';
    if ($C != '') {
...
as$Ie=>$lg) {
    ...
    page_header((isset($_GET['host']) ? 'Username'.': '.h("$ha@$_GET[host]") : 'Create user'), $n, ['privileges'=>['', 'Privileges']]);
    if ($_POST) {
        $K = $_POST;
        $Zc = $Be;
    } else {
        ...
        $Zc[(DB == '' || $Zc ? '' : idf_escape(addcslashes(DB, '%_\\'))).'.*'] = [];
    }
    echo'<form action="" method="post">
<table cellspacing="0">
...
';
    ...
    foreach ($Zc
as$Ie=>$Yc) {
        echo'<th>'.($Ie != '*.*' ? "<input name='objects[$s]' value='".h($Ie)."' size='10' autocapitalize='off'>" : "<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");
        $s++;
    }
    echo"</thead>\n";
    foreach ([''=>'', 'Server Admin'=>'Server', 'Databases'=>'Database', 'Tables'=>'Table', 'Columns'=>'Column', 'Procedures'=>'Routine']as$xb=>$Nb) {
...
';
    ...
    if (support('kill') && $_POST && !$n) {
        ...
        queries_redirect(ME.'processlist=', lang(['%d process has been killed.', '%d processes have been killed.'], $Od), $Od || !$_POST['kill']);
    }
...
as$y=>$X) {
        ...
        foreach ($K
as$y=> $X) {
            echo'<td>'.(($x == 'sql' && $y == 'Info' && preg_match('~Query|Killed~', $K['Command']) && $X != '') || ($x == 'pgsql' && $y == 'current_query' && $X != '<IDLE>') || ($x == 'oracle' && $y == 'sql_text' && $X != '') ? "<code class='jush-$x'>".shorten_utf8($X, 100, '</code>').' <a href="'.h(ME.($K['db'] != '' ? 'db='.urlencode($K['db']).'&' : '').'sql='.urlencode($X)).'">'.'Clone'.'</a>' : nbsp($X));
        }
        echo"\n";
    }
    echo'</table>
...
';
    ...
    echo'<h3>'.'Master status'.doc_link(['sql'=>'show-master-status.html'])."</h3>\n";
    ...
    if (!$fe) {
        ...
        echo"<table cellspacing='0'>\n";
        foreach ($fe[0]as$y=>$X) {
            echo'<tr>','<th>'.h($y),'<td>'.nbsp($X);
        }
        echo"</table>\n";
    }
    $Kg = replication_status('SLAVE');
    if ($Kg) {
        echo'<h3>'.'Slave status'.doc_link(['sql'=>'show-slave-status.html'])."</h3>\n";
        foreach ($Kg[0]as$Jg) {
            echo"<table cellspacing='0'>\n";
            foreach ($Jg
as$y=>$X) {
...
as$C=>$X) {
                ...
                if ($_POST['delete'] || $O) {
                    ...
                    if ($_POST['all'] || ($Uh === [] && is_array($_POST['check'])) || $Cd) {
                ...
                $pe = lang(['%d item has been affected.', '%d items have been affected.'], $ya);
...
as$y=>$X) {
                        ...
                        $I = $Vb->update($a, $O, ' WHERE '.($Z ? implode(' AND ', $Z).' AND ' : '').where_check($Sh, $p), !($Cd || $Uh === []), ' ');
                        if (!$I) {
                            break;
                        }
                        $ya += $g->affected_rows;
                    }
                    queries_redirect(remove_from_uri(), lang(['%d item has been affected.', '%d items have been affected.'], $ya), $I);
                }
                ...
                queries_redirect(remove_from_uri('page'), lang(['%d row has been imported.', '%d rows have been imported.'], $ya), $I);
                $Vb->rollback();
            }
        }
    }
    ...
    if (!$f && support('table')) {
        ...
        if (!$I) {
            ...
            if (!$L) {
                ...
                echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n",'<thead><tr>'.(!$ad && $M ? '' : "<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);' class='jsonly'> <a href='".h($_GET['modify'] ? remove_from_uri('modify') : $_SERVER['REQUEST_URI'].'&modify=1')."'>".'Modify'.'</a>');
                $Ae = [];
                $Xc = [];
                reset($M);
                $Vf = 1;
                foreach ($L[0]as$y=>$X) {
                    if ($y != $Ke) {
                        ...
                        if ($C != '') {
                            ...
                            $Nb = '&desc%5B0%5D=1';
                            echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($ld.($af[0] == $e || $af[0] == $y || (!$af && $Cd && $ad[0] == $e) ? $Nb : '')).'">';
                            echo
apply_sql_function($X['fun'], $C).'</a>';
                            echo"<span class='column hidden'>","<a href='".h($ld.$Nb)."' title='".'descending'."' class='text'> â†“</a>";
                            if (!$X['fun']) {
                                echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($y)).'\'); return false;" title="'.'Search'.'" class="text jsonly"> =</a>';
                            }
                            echo'</span>';
                        }
                        $Xc[$y] = $X['fun'];
...
pagination($s, $E);
                ...
                echo"<p class='count'>\n",($Uc !== false ? '('.($uc ? '' : '~ ').lang(['%d row', '%d rows'], $Uc).') ' : '');
                $Sb = ($uc ? '' : '~ ').$Uc;
                echo
...
';
            ...
            if ($b->selectImportPrint()) {
                ...
                echo"<input type='file' name='csv_file'> ",html_select('separator', ['csv'=>'CSV,', 'csv;'=>'CSV;', 'tsv'=>'TSV'], $xa['format'], 1);
    ...
    if (!$hi) {
        ...
        foreach ($hi
as$y=>$X) {
            echo'<tr>',"<th><code class='jush-".$x.($P ? 'status' : 'set')."'>".h($y).'</code>','<td>'.nbsp($X);
        }
        echo"</table>\n";
    }
    ...
    header('Content-Type: text/javascript; charset=utf-8');
    if ($_GET['script'] == 'db') {
        $ah = ['Data_length'=>0, 'Index_length'=>0, 'Data_free'=>0];
        foreach (table_status()as$C=>$S) {
            ...
            if (!is_view($S)) {
                ...
                foreach ($ah + ['Auto_increment'=>0, 'Rows'=>0]as$y=>$X) {
                    if ($S[$y] != '') {
                        $X = format_number($S[$y]);
    ...
    if ($b->homepage()) {
        if ($_GET['ns'] !== '') {
            ...
            if (!$ih) {
                ...
                $Tb = doc_link(['sql'=>'show-table-status.html']);
                echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);" class="jsonly">','<th>'.'Table','<td>'.'Engine'.doc_link(['sql'=>'storage-engines.html']),'<td>'.'Collation'.doc_link(['sql'=>'charset-mysql.html']),'<td>'.'Data Length'.$Tb,'<td>'.'Index Length'.$Tb,'<td>'.'Data Free'.$Tb,'<td>'.'Auto Increment'.doc_link(['sql'=>'example-auto-increment.html']),'<td>'.'Rows'.$Tb,(support('comment') ? '<td>'.'Comment'.$Tb : ''),"</thead>\n";
                $T = 0;
                foreach ($ih
as$C=>$U) {
                    $ki = ($U !== null && !preg_match('~table~i', $U));
                    $t = h('Table-'.$C);
                    echo'<tr'.odd().'><td>'.checkbox(($ki ? 'views[]' : 'tables[]'), $C, in_array($C, $jh, true), '', "formUncheck('check-all');", '', $t),'<th>'.(support('table') || support('indexes') ? "<a href='".h(ME).'table='.urlencode($C)."' title='".'Show structure'."' id='$t'>".h($C).'</a>' : h($C));
                    if ($ki) {
                        ...
                        foreach (['Engine'=>[], 'Collation'=>[], 'Data_length'=>['create', 'Alter table'], 'Index_length'=>['indexes', 'Alter indexes'], 'Data_free'=>['edit', 'New item'], 'Auto_increment'=>['auto_increment=1&create', 'Alter table'], 'Rows'=>['select', 'Select data']]as$y=>$_) {
                            $t = " id='$y-".h($C)."'";
                ...
                echo'<tr><td>&nbsp;<th>'.sprintf('%d in total', count($ih)),'<td>'.nbsp($x == 'sql' ? $g->result('SELECT @@storage_engine') : ''),'<td>'.nbsp(db_collation(DB, collations()));
                ...
                if (!information_schema(DB)) {
                    ...
                    if (count($l) != 1 && $x != 'sqlite') {
                        ...
                        echo'<p>'.'Move to other database'.': ',($l ? html_select('target', $l, $m) : '<input name="target" value="'.h($m).'" autocapitalize="off">')," <input type='submit' name='move' value='".'Move'."'>",(support('copy') ? " <input type='submit' name='copy' value='".'Copy'."'>" : ''),"\n";
                    }
            ...
            if (support('routine')) {
                ...
                if ($qg) {
                    ...
                    foreach ($qg
as$K) {
                        echo'<tr'.odd().'>','<th><a href="'.h(ME).($K['ROUTINE_TYPE'] != 'PROCEDURE' ? 'callf=' : 'call=').urlencode($K['ROUTINE_NAME']).'">'.h($K['ROUTINE_NAME']).'</a>','<td>'.h($K['ROUTINE_TYPE']),'<td>'.h($K['DTD_IDENTIFIER']),'<td><a href="'.h(ME).($K['ROUTINE_TYPE'] != 'PROCEDURE' ? 'function=' : 'procedure=').urlencode($K['ROUTINE_NAME']).'">'.'Alter'.'</a>';
                    }
                    echo"</table>\n";
                }
...
as$K) {
                    ...
                    if ($sc && $sc != 'ON') {
                        echo"<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($sc)."\n";
                    }
                }
